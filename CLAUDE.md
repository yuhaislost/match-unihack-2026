...
**Data fetching:**
- Components receive data as **props**. They do not fetch their own data.
- Data fetching happens in **server components** (page-level) or via **tRPC queries in hooks**. The fetching component passes results down as props.
...
|---|---|---|
| **Server component** (page/layout) | tRPC server caller (`@/trpc/server`) | `const venues = await trpc.venue.listNearby({ lat, lng, radius });` |
| **Client component** | tRPC + TanStack React Query hooks (`@/trpc/client`) | `const { data } = trpc.venue.listNearby.useQuery({ lat, lng, radius });` |
| **Mutation (client)** | tRPC `useMutation` hook | `trpc.booking.create.useMutation()` |
...
- Server components should `await` tRPC calls. Client components use the React Query hooks for automatic caching, deduplication, and loading states.
